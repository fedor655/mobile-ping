...
замера к устройству:
  ...
  * ICMP под Windows — только исходный адрес: у IcmpSendEcho2Ex параметра для
    интерфейса нет. Путь проверяется icmp_will_leak() прямым измерением:
    пакет с TTL=1 отбивает первый роутер, и это обязан быть шлюз устройства.
"""
...
def icmp_will_leak(src_ip, gateways=None, dest_ip="1.1.1.1"):
    """
    Уйдут ли ICMP мимо нужного интерфейса.

    Возвращает None, если пинги идут через устройство, иначе описание того,
    куда они уходят.

    Под Linux ping-сокет прибивается к интерфейсу через IP_UNICAST_IF, и
    вопроса нет. Под Windows у IcmpSendEcho2Ex есть только исходный адрес, и
    это проверяется прямым измерением: пакет с TTL=1 отбивает первый роутер на
    пути, и если это не шлюз устройства — пинги утекают.

    Раньше здесь спрашивали у ядра, каким интерфейсом оно отправит пакет по
    умолчанию. Это было ошибкой: по умолчанию всегда побеждает домашняя сеть,
    и под Windows отбраковывался бы любой телефон, пока подключён дом. Замер
    показал обратное: с заданным исходным адресом ICMP идёт через шлюз
    телефона, даже когда маршрут по умолчанию ведёт в кабель.
    """
    if ICMP_BOUND_TO_INTERFACE:
        return None
    from icmp_windows import first_hop
    hop, status = first_hop(dest_ip, src_ip)
    gateways = [g for g in (gateways or []) if g]
    if hop and gateways and hop not in gateways:
        return "первым ответил %s, а шлюз устройства %s" % (hop, ", ".join(gateways))
    if hop is None:
        # Шлюз не прислал «TTL истёк» — путь не виден. Само по себе это не
        # утечка, но если поднят полнотуннельный VPN, рисковать нельзя.
        import netinfo
        tunnels = netinfo.capturing_tunnels()
        if tunnels:
            return ("не удалось проверить путь ICMP (%s), а поднят VPN %s"
                    % (status, ", ".join(t["name"] for t in tunnels)))
    return None
